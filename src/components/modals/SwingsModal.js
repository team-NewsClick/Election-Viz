  ...
  return (
    <div className="flex flex-row relative w-full">
      ...
      <div className="relative w-11/12">
        <input type="range" id="input_A" min={-25} max={25} value={valueSwing_A} className="absolute h-4 w-full opacity-0" style={{zIndex: 2, left: 0}} onChange={(e) => setValueSwing_A(e.target.value)} />
        <div id="slider_A" className="relative h-2 bg-gray-300" style={{zIndex: 1}}>
          <div className="left-0 right-0 top-0 bottom-0 rounded-full" />
          <div id="range_A" className="absolute left-1/4 right-1/4 top-0 bottom-0 bg-blue-500 rounded-full" style={{zIndex: 2}} />
          ...
          <div id="thumb_A" className="absolute w-4 h-4 bg-blue-800 rounded-full opacity-100 left-1/4" style={{transform: "translate(-0.5rem, -0.25rem)", left: 0, zIndex: 3}} />
          ...
          <div id="valueSwingDisaply_A" className="absolute rounded-full left-1/4 font-bold" style={{transform: "translate(-0.5rem, -2rem)", left: 0}}>{valueSwing_A > 0 ? "+" + valueSwing_A: valueSwing_A}%</div>
        </div>
      </div>
    </div>
    )
